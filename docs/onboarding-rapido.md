...
La coordinación se reparte entre `proxy.py` (API/flujo) y `k8s_orchestrator.py` (orquestación Kubernetes): escalado dinámico, readiness, port-forward, ejecución en cascada y scale-down a cero.

## 2) Componentes clave

- `index.html` / `styles.css`: UI
- `script.js`: lógica cliente + terminal embebido (SSE)
- `proxy.py`: API Flask y coordinación de la operación
- `k8s_orchestrator.py`: orquestación Kubernetes (`kubectl scale/wait/port-forward`)
- `helm/suma-basica`: chart Helm (manifiestos parametrizados)
- `scripts/helm-local.ps1`, `scripts/helm-status.ps1`, `scripts/helm-clean.ps1`, `scripts/helm-all.ps1`: operación local Helm
...
# Desplegar recursos con Helm (recomendado)
