# Documento Técnico: proxy.py + k8s_orchestrator.py

## 1. Propósito

`proxy.py` es la capa API y de coordinación de la aplicación. La orquestación Kubernetes está desacoplada en `k8s_orchestrator.py`.

En conjunto ambos módulos resuelven:
...
- `proxy.py` delega en `K8sOrchestrator` la ejecución de `kubectl`.
- `k8s_orchestrator.py` escala deployments `suma-digito-{0..3}` con `kubectl scale`.
- `k8s_orchestrator.py` espera readiness con `kubectl wait` por label `app=suma-backend,digito={n}`.
- `k8s_orchestrator.py` crea/detiene `port-forward` por pod/servicio.
- `k8s_orchestrator.py` gestiona colisiones de puertos locales con asignación dinámica.
...
- `base_port` en `K8sOrchestrator`: puertos preferidos locales 31000-31003.
...
- `port_forward_processes` y `port_forward_ports` viven en `K8sOrchestrator`.
...
`k8s_orchestrator.py::establecer_port_forward(digito)`:
...
`k8s_orchestrator.py::detener_port_forward(digito)`:
...
`proxy.py::escalar_a_cero_en_background()`:

- Espera `SCALE_DOWN_DELAY_SECONDS`.
- Invoca `K8sOrchestrator.escalar_a_cero()` que recorre `MAX_DIGITOS` completo (0..3), escala cada deployment a 0 y detiene sus port-forwards.
